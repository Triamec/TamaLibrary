...
/* Overview
 * --------
 * This pulse generator Tama program is able to output periodical pulses at TSD Device Test Ax0..Ax1 DigIn1..6,
 * STO1..2 and DigIn7
 ...
 * 
 * Inputs:
 * Tama.Variables.GenPurposeVar0:	 Pulse period [s]
 * Tama.Variables.GenPurposeVar1:	 Pulse width [s]. If greater than or equal to Pulse period, an IndexOutOfRange error occurs.
 * Tama.Variables.GenPurposeIntVar0: A bit combination identifying the outputs to use. 
 *									 (B0..B5:  Ax0:DigIn1..6; B6..B7: Sto1..2
 *									  B8..B13: Ax1:DigIn1..6; B6: DigIn7)
 * Tama.Variables.GenPurposeIntVar1: Negative logic: Set to 0 if pulse is to be high. Set to 1 if pulse is to be low.
...
using Triamec.Tama.Rlid10;  // TSD Device Tester
using Triamec.Tama.Vmid5;

/// <summary>
/// Tama program for the TSD Device Tester to generate pulses on one or several outputs.
/// </summary>
[Tama]
static class PulseGeneratorTsdDevTest {
	#region Constants
	/// <summary>
	...
	/// <summary>
	/// Number of supported outputs per axis (Ax0: DigIn1..6 + Sto1..2; Ax1: DigIn1..6 + DigIn7)
	/// <para>The value is 8.</para>
	/// </summary>
	const int OutputsPerAxis = 8;
	/// <summary>
	...
	static void SetDigitalOutput(int outIdx, int value) {
		var pinIdx = OutputPinIdx[outIdx];
		switch (OutputAxisIdx[outIdx]) {
			case 0:
				// control Ax0DigIn1..6 and STOCh1..2
				if (pinIdx >= 0 && pinIdx <= 5) {
					Register.General.Commands.DigitalIo.TestVoltagesDac24.Axis0DigIn[pinIdx] = value > 0 ? 24.0f : 0f;
				} else if (pinIdx >= 6 && pinIdx <= 7) {
					Register.General.Commands.DigitalIo.TestVoltagesDac24.StoCh[pinIdx-6] = value > 0 ? 24.0f : 0f;
				}
				break;
			case 1:
				// control Ax1DigIn1..6 and DigIn7
				if (pinIdx >= 0 && pinIdx <= 5) {
					Register.General.Commands.DigitalIo.TestVoltagesDac24.Axis1DigIn[pinIdx] = value > 0 ? 24.0f : 0f;
				} else if (pinIdx == 6) {
					Register.General.Commands.DigitalIo.TestVoltagesDac24.DigIn7 = value > 0 ? 24.0f : 0f;
				}
				break;
			default:
				// unused 
